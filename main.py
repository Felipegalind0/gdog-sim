...
from commands import CommandState, TuneRegistry, _execute_suspension_command
...
# Static default wheel placement bias at rest (0.0 = centered).
STANCE_SHIFT_STATIC_BIAS_X_M = 0.0
...
# Register tunable variables: (name, initial_value, min, max, step)
tune.register("STANCE_SHIFT_MAX_LEG_X_M", STANCE_SHIFT_MAX_LEG_X_M, 0.0, 0.5, 0.01)
tune.register("STANCE_SHIFT_STATIC_BIAS_X_M", STANCE_SHIFT_STATIC_BIAS_X_M, -0.15, 0.15, 0.005)
tune.register("STANCE_SHIFT_CMD_SPEED_MAX_MPS", STANCE_SHIFT_CMD_SPEED_MAX_MPS, 0.5, 10.0, 0.1)
tune.register("STANCE_SHIFT_SPEED_ERROR_TO_ACCEL_GAIN", STANCE_SHIFT_SPEED_ERROR_TO_ACCEL_GAIN, 0.5, 20.0, 0.5)
tune.register("STANCE_SHIFT_ACCEL_MAX_MPS2", STANCE_SHIFT_ACCEL_MAX_MPS2, 1.0, 30.0, 0.5)
tune.register("STANCE_SHIFT_TILT_TO_LEG_X_GAIN", STANCE_SHIFT_TILT_TO_LEG_X_GAIN, 0.01, 1.0, 0.01)
tune.register("STANCE_SHIFT_FILTER_ALPHA", STANCE_SHIFT_FILTER_ALPHA, 0.05, 1.0, 0.05)
tune.register("STANCE_SHIFT_RISK_REDUCTION_GAIN", STANCE_SHIFT_RISK_REDUCTION_GAIN, 0.0, 2.0, 0.05)
tune.register("DRIVE_INPUT_VX_GAIN", DRIVE_INPUT_VX_GAIN, 0.5, 5.0, 0.05)
tune.register("DRIVE_ACCEL_RESPONSE_GAIN", DRIVE_ACCEL_RESPONSE_GAIN, 0.1, 10.0, 0.1)
tune.register("DRIVE_BRAKE_RESPONSE_GAIN", DRIVE_BRAKE_RESPONSE_GAIN, 0.1, 10.0, 0.1)
tune.register("DRIVE_BRAKE_PREDICTIVE_GAIN", DRIVE_BRAKE_PREDICTIVE_GAIN, 0.0, 5.0, 0.1)
tune.register("DRIVE_POWER_SCALE_MIN", DRIVE_POWER_SCALE_MIN, 0.5, 10.0, 0.25)
tune.register("DRIVE_POWER_SCALE_MAX", DRIVE_POWER_SCALE_MAX, 1.0, 20.0, 0.5)
tune.register("DRIVE_BRAKE_THROTTLE_MIN", DRIVE_BRAKE_THROTTLE_MIN, 0.0, 1.0, 0.05)
tune.register("DRIVE_BRAKE_REVERSE_SCALE", DRIVE_BRAKE_REVERSE_SCALE, 0.0, 2.0, 0.05)
tune.register("ANTI_TIP_PITCH_DAMP_GAIN", ANTI_TIP_PITCH_DAMP_GAIN, 0.0, 1.0, 0.02)
tune.register("ANTI_TIP_PITCH_RESTORE_GAIN", ANTI_TIP_PITCH_RESTORE_GAIN, 0.0, 2.0, 0.05)
...
def main():
    ...
    stance_shift_target_x = float(STANCE_SHIFT_STATIC_BIAS_X_M)
    stance_shift_leg_x = float(STANCE_SHIFT_STATIC_BIAS_X_M)
    ...
    try:
        ...
        while steps <= 0 or i < steps:
            # Apply live-tuned variable values to module globals.
            for _tk, _tv in tune.snapshot().items():
                globals()[_tk] = _tv
            ...
            stance_target_uncapped = (
                STANCE_SHIFT_STATIC_BIAS_X_M
                + STANCE_SHIFT_TILT_TO_LEG_X_GAIN * desired_tilt_target
            )
            stance_scale = float(
                np.clip(
